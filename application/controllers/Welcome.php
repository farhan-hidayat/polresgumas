...
class Welcome extends CI_Controller
{
	...
	{
		// 3 artikel terbaru
		// $data['artikel'] = $this->db->query("SELECT * FROM artikel,pengguna,kategori WHERE status_artikel='publish' AND artikel_author=pengguna_id AND artikel_kategori=kategori_id ORDER BY artikel_id DESC LIMIT 3")->result();
	...
	public function gallery()
	{
		...
		$data['gallery'] = $this->db->query("SELECT * FROM gallery,pengguna,kategori WHERE status='publish' AND author=pengguna_id AND kategori=kategori_id ORDER BY gallery_id DESC LIMIT $config[per_page] OFFSET $from")->result();
	...
	public function tutorial()
	{
		...
		// $data['gallery'] = $this->db->query("SELECT * FROM gallery,pengguna,kategori WHERE status='publish' AND author=pengguna_id AND kategori=kategori_id ORDER BY gallery_id DESC LIMIT $config[per_page] OFFSET $from")->result();
		$data['tutorial'] = $this->m_data->get_data('tutorial')->result();
		$this->load->view('frontend/layout/v_header', $data);
		$this->load->view('frontend/v_tutorial', $data);
		$this->load->view('frontend/layout/v_footer', $data);
	}
